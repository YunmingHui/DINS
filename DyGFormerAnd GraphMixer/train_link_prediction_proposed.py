...
if __name__ == "__main__":
    ...
    node_raw_features, edge_raw_features, full_data, train_data, val_data, test_data = \
        get_link_prediction_data(dataset_name=args.dataset_name, val_index=args.val_index, test_index=args.test_index)
    ...
    val_idx_data_loader = get_idx_data_loader(indices_list=list(range(len(val_data.src_node_ids))), batch_size=200, shuffle=False)
    test_idx_data_loader = get_idx_data_loader(indices_list=list(range(len(test_data.src_node_ids))), batch_size=200, shuffle=False)
    ...
    for run in range(args.num_runs):
        ...
        for i in range(len(train_data.src_node_ids)):
            if (train_data.src_node_ids[i], train_data.dst_node_ids[i]) not in train_node_pairs2timestamps:
                train_node_pairs2timestamps[(train_data.src_node_ids[i], train_data.dst_node_ids[i])] = []
            train_node_pairs2timestamps[(train_data.src_node_ids[i], train_data.dst_node_ids[i])].append(train_data.node_interact_times[i])
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader):
                ...
                sources_batch_appear_after, destinations_batch_appear_after, timestamps_batch_appear_after = [], [], []
                for edge_index in range(train_data_indices[-1]+1,len(train_data.src_node_ids)):
                    if (train_data.src_node_ids[edge_index], train_data.dst_node_ids[edge_index]) in batch_node_pairs:
                        sources_batch_appear_after.append(train_data.src_node_ids[edge_index])
                        destinations_batch_appear_after.append(train_data.dst_node_ids[edge_index])
                        timestamps_batch_appear_after.append(train_data.node_interact_times[edge_index])
                    if len(sources_batch_appear_after) == len(batch_src_node_ids):
                        break
                
                # Positive samples for positive enhancement
                sources_batch_neg_temporal, destinations_batch_neg_temporal, timestamps_batch_neg_temporal = [], [], []
                for i in range(len(batch_src_node_ids)):
                    range_start = batch_node_interact_times[i] + 1
                    range_end = min(train_data.node_interact_times[-1], range_start + 500) 
                    if (range_end - range_start) <= 10:
                        continue
                    interval = (range_end - range_start) // 5 
                    for j in range(5):
                        used_timestamps = set(train_node_pairs2timestamps[(batch_src_node_ids[i], batch_dst_node_ids[i])])
                        for attempt in range(100):  
                            ...
                            if ts not in used_timestamps:
                                ...
                                destinations_batch_neg_temporal.append(batch_dst_node_ids[i])
                ...
                if args.model_name in ['GraphMixer']:
                    ...
                    if len(negative_source_III)>0:
                        ...
                        batch_neg_src_node_embeddings_III, batch_neg_dst_node_embeddings_III = \
                            model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=np.array(negative_source_III),
                    ...
                    if len(sources_batch_appear_after)>0:
                        ...
                        batch_neg_src_node_embeddings_IV, batch_neg_dst_node_embeddings_IV = \
                            model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=np.array(sources_batch_appear_after),
                    ...
                    if len(sources_batch_neg_temporal)>0:
                        ...
                        batch_neg_src_node_embeddings_V, batch_neg_dst_node_embeddings_V = \
                            model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=np.array(sources_batch_neg_temporal),
                ...
                negative_probabilities = model[1](input_1=batch_src_node_embeddings, input_2=batch_neg_dst_node_embeddings).squeeze(dim=-1).sigmoid()
                predicts = torch.cat([positive_probabilities, negative_probabilities], dim=0)
                labels = torch.cat([torch.ones_like(positive_probabilities), torch.zeros_like(negative_probabilities)], dim=0)
                loss += loss_func(input=predicts, target=labels)

                negative_probabilities = model[1](input_1=batch_neg_src_node_embeddings, input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
                labels = torch.zeros_like(negative_probabilities)
                loss += loss_func(input=negative_probabilities, target=labels)

                if len(negative_source_III)>0:
                    negative_probabilities = model[1](input_1=batch_neg_src_node_embeddings_III, input_2=batch_neg_dst_node_embeddings_III).squeeze(dim=-1).sigmoid()
                    labels = torch.zeros_like(negative_probabilities)
                    loss += loss_func(input=negative_probabilities, target=labels)
                
                if len(sources_batch_appear_after)>0:
                    positive_probabilities = model[1](input_1=batch_neg_src_node_embeddings_IV, input_2=batch_neg_dst_node_embeddings_IV).squeeze(dim=-1).sigmoid()
                    labels = torch.ones_like(positive_probabilities)
                    loss += loss_func(input=positive_probabilities, target=labels)

                if len(sources_batch_neg_temporal)>0:
                    negative_probabilities = model[1](input_1=batch_neg_src_node_embeddings_V, input_2=batch_neg_dst_node_embeddings_V).squeeze(dim=-1).sigmoid()
                    labels = torch.zeros_like(negative_probabilities)
                    loss += loss_func(input=negative_probabilities, target=labels)
            ...
            overall_ap, overall_auc, \
            hist_6h_ap, hist_6h_auc, hist_12h_ap, hist_12h_auc, \
            hist_24h_ap, hist_24h_auc, random_sender_ap, random_sender_auc, \
            random_receiver_ap, random_receiver_auc, loop_ap, loop_auc = evaluate_model_link_prediction(model_name=args.model_name,
                                                                                                        model=model,
