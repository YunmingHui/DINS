...
def evaluate_model_link_prediction(
    model_name: str,
    model: nn.Module,
    ...
    with torch.no_grad():
        ...
        for evaluate_data_indices in evaluate_idx_data_loader:
            ...
            (
                batch_src_loop_node_ids,
                batch_dst_loop_node_ids,
                batch_loop_node_interact_times,
            ) = ([], [], [])
            (
                batch_src_no_loop_node_ids,
                batch_dst_no_loop_node_ids,
                batch_no_loop_node_interact_times,
            ) = ([], [], [])
            ...
            for i in range(len(batch_src_node_ids)):
                if batch_src_node_ids[i] == batch_dst_node_ids[i]:
                    ...
                    batch_no_loop_node_interact_times.append(
                        batch_node_interact_times[i]
                    )
            ...
            batch_no_loop_node_interact_times = np.array(
                batch_no_loop_node_interact_times
            )

            batch_neg_src_node_ids, batch_neg_dst_node_ids = (
                evaluate_neg_edge_sampler.sample(size=len(batch_dst_no_loop_node_ids))
            )
            ...
            if model_name in ["GraphMixer"]:
                ...
                (
                    batch_src_no_loop_node_embeddings,
                    batch_dst_no_loop_node_embeddings,
                ) = model[0].compute_src_dst_node_temporal_embeddings(
                    src_node_ids=batch_src_no_loop_node_ids,
                ...
                (
                    batch_neg_src_node_embeddings_6h,
                    batch_neg_dst_node_embeddings_6h,
                ) = model[0].compute_src_dst_node_temporal_embeddings(
                    src_node_ids=batch_src_node_ids,
                    dst_node_ids=batch_dst_node_ids,
                ...
                (
                    batch_neg_src_node_embeddings_12h,
                    batch_neg_dst_node_embeddings_12h,
                ) = model[0].compute_src_dst_node_temporal_embeddings(
                    src_node_ids=batch_src_node_ids,
                    dst_node_ids=batch_dst_node_ids,
                ...
                (
                    batch_neg_src_node_embeddings_24h,
                    batch_neg_dst_node_embeddings_24h,
                ) = model[0].compute_src_dst_node_temporal_embeddings(
                    src_node_ids=batch_src_node_ids,
                    dst_node_ids=batch_dst_node_ids,
            ...
            negative_probabilities_6h = (
                model[1](
                    input_1=batch_neg_src_node_embeddings_6h,
                    input_2=batch_neg_dst_node_embeddings_6h,
                )
                .squeeze(dim=-1)
                .sigmoid()
            )
            negative_probabilities_12h = (
                model[1](
                    input_1=batch_neg_src_node_embeddings_12h,
                    input_2=batch_neg_dst_node_embeddings_12h,
                )
                .squeeze(dim=-1)
                .sigmoid()
            )
            negative_probabilities_24h = (
                model[1](
                    input_1=batch_neg_src_node_embeddings_24h,
                    input_2=batch_neg_dst_node_embeddings_24h,
                )
                .squeeze(dim=-1)
                .sigmoid()
            )
            negative_probabilities_loop = (
                model[1](
            ...
            if overall_predict.size == 0:
                ...
                overall_predict = np.concatenate(
                    [overall_predict, positive_probabilities_all.cpu().detach().numpy()]
                )
            ...
            overall_predict = np.concatenate(
                [
                    overall_predict,
                    negative_probabilities_6h.cpu().detach().numpy(),
                    negative_probabilities_12h.cpu().detach().numpy(),
                    negative_probabilities_24h.cpu().detach().numpy(),
                    negative_probabilities_loop.cpu().detach().numpy(),
                ]
            )
            overall_label = np.concatenate(
                [
                    overall_label,
                    np.ones(len(positive_probabilities_all)),
                    np.zeros(len(negative_probabilities_6h)),
                    np.zeros(len(negative_probabilities_12h)),
                    np.zeros(len(negative_probabilities_24h)),
                    np.zeros(len(negative_probabilities_loop)),
                ]
            )
            ...
            predicts = (
                torch.cat(
                    [positive_probabilities_all, negative_probabilities_6h], dim=0
                )
                .cpu()
                .detach()
                .numpy()
            ...
            predicts = (
                torch.cat(
                    [positive_probabilities_all, negative_probabilities_12h], dim=0
                )
                .cpu()
                .detach()
                .numpy()
            ...
            predicts = (
                torch.cat(
                    [positive_probabilities_all, negative_probabilities_24h], dim=0
                )
                .cpu()
                .detach()
                .numpy()
            ...
            predicts = (
                torch.cat(
                    [positive_probabilities_all, negative_probabilities_loop], dim=0
                )
                .cpu()
                .detach()
                .numpy()
            ...
            positive_probabilities_non_loop = (
                model[1](
                    input_1=batch_src_no_loop_node_embeddings,
                    input_2=batch_dst_no_loop_node_embeddings,
                )
                .squeeze(dim=-1)
                .sigmoid()
            )
            negative_probabilities_swap = (
                model[1](
                    input_1=batch_dst_no_loop_node_embeddings,
                    input_2=batch_src_no_loop_node_embeddings,
                )
                .squeeze(dim=-1)
                .sigmoid()
            )
            negative_probabilities_random_recevier = (
                model[1](
                    input_1=batch_src_no_loop_node_embeddings,
            ...
            negative_probabilities_random_sender = (
                model[1](
            ...
            overall_predict = np.concatenate(
                [
                    overall_predict,
                    negative_probabilities_swap.cpu().detach().numpy(),
                    negative_probabilities_random_recevier.cpu().detach().numpy(),
                    negative_probabilities_random_sender.cpu().detach().numpy(),
                ]
            )
            overall_label = np.concatenate(
                [
                    overall_label,
                    np.zeros(len(negative_probabilities_swap)),
                    np.zeros(len(negative_probabilities_random_recevier)),
                    np.zeros(len(negative_probabilities_random_sender)),
                ]
            )

            labels = np.concatenate(
                [
                    np.ones(len(positive_probabilities_non_loop)),
                    np.zeros(len(negative_probabilities_random_recevier)),
                ]
            )
            predicts = (
