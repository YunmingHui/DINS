...
parser.add_argument(
    "--valid_index", type=int, help="The index of first edge in the validation set"
)
parser.add_argument(
    "--test_index", type=int, help="The index of first edge in the test set"
)
...
node_features, edge_features, full_data, train_data, val_data, test_data = get_data(
    DATA,
    VALID_INDEX,
    TEST_INDEX,
...
for i in range(args.n_runs):
    ...
    for i in range(len(train_data.sources)):
        if (
            train_data.sources[i],
            train_data.destinations[i],
        ) not in train_node_pairs2timestamps:
            train_node_pairs2timestamps[
                (train_data.sources[i], train_data.destinations[i])
            ] = []
        train_node_pairs2timestamps[
            (train_data.sources[i], train_data.destinations[i])
        ].append(train_data.timestamps[i])
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for k in range(0, num_batch, args.backprop_every):
            ...
            for j in range(args.backprop_every):
                ...
                node_pair_batch = set(
                    zip(sources_batch, destinations_batch)
                )  # set of node pairs in the batch
                ...
                if len(negative_source_loop) > 1:
                    ...
                    with torch.no_grad():
                        neg_label = torch.zeros(
                            len(neg_prob), dtype=torch.float, device=device
                        )
                    loss += criterion(neg_prob.squeeze(), neg_label)
                ...
                (
                    ...
                    edge_idxs_batch_appear_after,
                ) = ([], [], [], [])
                for edge_idx in range(end_idx + 1, len(train_data.sources)):
                    if (
                        train_data.sources[edge_idx],
                        train_data.destinations[edge_idx],
                    ) in node_pair_batch:
                        ...
                        timestamps_batch_appear_after.append(
                            train_data.timestamps[edge_idx]
                        )
                    if len(sources_batch_appear_after) == BATCH_SIZE:
                        break
                if len(sources_batch_appear_after) > 0:
                    pos_prob = tgn.compute_edge_probabilities_no_negative(
                        np.array(sources_batch_appear_after),
                    ...
                    with torch.no_grad():
                        pos_label = torch.ones(
                            len(sources_batch_appear_after),
                            dtype=torch.float,
                            device=device,
                        )
                ...
                for i in range(size):
                    ...
                    for j in range(5):
                        used_timestamps = set(
                            train_node_pairs2timestamps[
                                (sources_batch[i], destinations_batch[i])
                            ]
                        )
                ...
                if len(sources_batch_neg_temporal) > 0:
                    neg_prob = tgn.compute_edge_probabilities_no_negative(
                        np.array(sources_batch_neg_temporal),
                    ...
                    with torch.no_grad():
                        neg_label = torch.zeros(
                            len(sources_batch_neg_temporal),
                            dtype=torch.float,
                            device=device,
                        )
                    loss += criterion(neg_prob.squeeze(), neg_label)
        ...
        logger.info("12h val auc: {}, 12h val ap: {}".format(hist_12h_auc, hist_12h_ap))
        logger.info("24h val auc: {}, 24h val ap: {}".format(hist_24h_auc, hist_24h_ap))
        logger.info(
    ...
    logger.info("Test statistics -- auc: {}, ap: {}".format(overall_auc, overall_ap))
    logger.info("6h test auc: {}, 6h test ap: {}".format(hist_6h_auc, hist_6h_ap))
    logger.info("12h test auc: {}, 12h test ap: {}".format(hist_12h_auc, hist_12h_ap))
    logger.info("24h test auc: {}, 24h test ap: {}".format(hist_24h_auc, hist_24h_ap))
    logger.info(
