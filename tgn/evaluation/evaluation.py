...
def eval_edge_prediction(model, negative_edge_sampler, data, n_neighbors, batch_size=200):
  ...
  with torch.no_grad():
    ...
    for k in range(num_test_batch):
      ...
      sources_batch_no_loop, destinations_batch_no_loop, timestamps_batch_no_loop, edge_idxs_batch_no_loop = [], [], [], [] 
      sources_batch_loop, destinations_batch_loop, timestamps_batch_loop, edge_idxs_batch_loop = [], [], [], [] 
      sources_batch_all, destinations_batch_all, timestamps_batch_all, edge_idxs_batch_all = [], [], [], [] 
      ...
      pos_prob, neg_prob = model.compute_edge_probabilities(sources_batch_all, destinations_batch_all, negatives_batch,
                                                            timestamps_batch_all, edge_idxs_batch_all, n_neighbors)
      ...
      neg_prob = model.compute_edge_probabilities_no_negative(sources_batch_all, destinations_batch_all,
                                                            timestamps_batch_all + 6*60/5, edge_idxs_batch_all, n_neighbors)
      ...
      hist_6h_ap.append(average_precision_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      hist_6h_auc.append(roc_auc_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))

      # Predict negative edges generated by adding 12 hours to the timestamps
      neg_prob = model.compute_edge_probabilities_no_negative(sources_batch_all, destinations_batch_all,
                                                            timestamps_batch_all + 12*60/5, edge_idxs_batch_all, n_neighbors)
      true_label = np.concatenate([np.ones(batch_size), np.zeros(batch_size)])
      overall_label = np.concatenate([overall_label, np.zeros(batch_size)])
      overall_prob = np.concatenate([overall_prob, neg_prob.cpu().numpy()])

      hist_12h_ap.append(average_precision_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      hist_12h_auc.append(roc_auc_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))

      # Predict negative edges generated by adding 24 hours to the timestamps
      neg_prob = model.compute_edge_probabilities_no_negative(sources_batch_all, destinations_batch_all,
                                                            timestamps_batch_all + 24*60/5, edge_idxs_batch_all, n_neighbors)
      true_label = np.concatenate([np.ones(batch_size), np.zeros(batch_size)])
      overall_label = np.concatenate([overall_label, np.zeros(batch_size)]) 
      overall_prob = np.concatenate([overall_prob, neg_prob.cpu().numpy()])

      hist_24h_ap.append(average_precision_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      hist_24h_auc.append(roc_auc_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      ...
      neg_prob = model.compute_edge_probabilities_no_negative(np.array(sample_nodes), np.array(sample_nodes),
                                                              timestamps_batch_all, edge_idxs_batch_all, n_neighbors)
      ...
      loop_ap.append(average_precision_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      loop_auc.append(roc_auc_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      ...
      neg_prob = model.compute_edge_probabilities_no_negative(sources_batch_no_loop, negative_samples,
                                                            timestamps_batch_no_loop, edge_idxs_batch_no_loop, n_neighbors)
      overall_label = np.concatenate([overall_label, np.zeros(batch_no_loop_size)])
      overall_prob = np.concatenate([overall_prob, neg_prob.cpu().numpy()])

      true_label = np.concatenate([np.ones(len(pos_prob)), np.zeros(len(neg_prob))])
      random_receiver_ap.append(average_precision_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      random_receiver_auc.append(roc_auc_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))

      # Predict negative edges generated by randomly changing sender
      _, negative_samples = negative_edge_sampler.sample(batch_no_loop_size)
      neg_prob = model.compute_edge_probabilities_no_negative(negative_samples, destinations_batch_no_loop,
                                                            timestamps_batch_no_loop, edge_idxs_batch_no_loop, n_neighbors)
      overall_label = np.concatenate([overall_label, np.zeros(batch_no_loop_size)])
      overall_prob = np.concatenate([overall_prob, neg_prob.cpu().numpy()])

      true_label = np.concatenate([np.ones(len(pos_prob)), np.zeros(len(neg_prob))])
      random_sender_ap.append(average_precision_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
      random_sender_auc.append(roc_auc_score(true_label, np.concatenate([pos_prob.cpu().numpy(), neg_prob.cpu().numpy()])))
